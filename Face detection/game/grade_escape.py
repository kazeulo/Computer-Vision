...
# Function to draw enemies (Kwatro and Singko)
...
# Function to draw boosters (Passing grades)
...
# Function to move enemies (Kwatro and Singko)
...
def move_boosters(boosters):
    ...
    for booster in boosters:
        ...
        if spaceship_x < booster[0] < spaceship_x + spaceship_width and spaceship_y < booster[1] < spaceship_y + spaceship_height:
            score += 3  # Increase score by 3 when booster is collected
            speed_boost = True  # Activate speed boost
            boost_duration = 100  # Boost lasts for 100 frames
            new_boosters.remove(booster)
    return new_boosters

# Function to detect smile (triggering firing)
def detect_smile(frame, face_coords):
    ...
    # Trigger firing when a smile is detected
    return len(smiles) > 0
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        # Check for smile and fire at enemies if detected
        if detect_smile(frame, (x, y, w, h)):
            print("Firing at obstacles!")  # Placeholder for firing action
    ...
    # Randomly add new enemies (Kwatro and Singko)
    ...
    # Draw enemies (Kwatro and Singko)
